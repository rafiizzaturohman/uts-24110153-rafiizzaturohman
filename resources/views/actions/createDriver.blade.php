    ...
    <div class="bg-gray-600 p-3 rounded-sm text-white">
        <form action="{{ route('storeDriver') }}" method="POST">
            ...
            <div class="flex flex-col my-2 space-y-2">
                ...
                <input type="text" class="border-b-2 border-[#7A8BA2] rounded-md px-2 py-1.5 focus:border-b-2 focus:border-white" name="kd_driver" id="kd_driver" required>

            </div>
            <div class="flex flex-col my-2 space-y-2">
                <label for="name" class="font-bold tracking-wider">Name</label>
                <input type="text" class="border-b-2 border-[#7A8BA2] rounded-md px-2 py-1 focus:border-b-2 focus:border-white" name="name" id="name" required>

            </div>
            <div class="flex flex-col my-2 space-y-2">
                <label for="phone" class="font-bold tracking-wider">Phone</label>
                <input type="text" class="border-b-2 border-[#7A8BA2] rounded-md px-2 py-1 focus:border-b-2 focus:border-white" name="phone" id="phone" required>

            </div>
            <div class="flex flex-col my-2 space-y-2">
                <label for="order_num" class="font-bold tracking-wider">Order Number</label>
                <input type="text" class="border-b-2 border-[#7A8BA2] rounded-md px-2 py-1 focus:border-b-2 focus:border-white" name="order_num" id="order_num" required>

            </div>

            <button class="cursor-pointer my-2 px-2 py-1 rounded-md bg-teal-700 hover:bg-teal-600 transition ease-in-out duration-200 font-bold tracking-wide" type="submit">Add</button>
        </form>
    </div>
</div>
